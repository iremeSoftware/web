  ...
  <div class="w-full flex" >
    ...
    <div class="w-full md:w-5/6">
      ...
      <DataTable>
        ...
        <tbody v-if="getClassroomList.classrooms?.length > 0" >
            <tr v-for="(classroom,i) in getClassroomList.classrooms" :key="classroom.id" class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                <input v-if="i==0" type="hidden" v-set="firstItem = (i + getClassroomList.offset) +1">
                <input  type="hidden" v-set="lastItem = (i + getClassroomList.offset) +1">
                <td class="px-4 md:px-6 py-4 border">
                   {{ i+getClassroomList.offset+1}}
                </td>
                <td class="px-4 md:px-6 py-4 border">
                ...
                <td class="px-4 md:px-6 py-4 border">
                   {{ classroom.classroom_alias !="" ? classroom.classroom_alias : "Not defined" }}
                </td>
                <td class="px-4 md:px-6 py-4 border">
                   {{ classroom.name }}
                </td>
                <td class="flex px-4 md:px-6 py-4 colToHide space-x-2 md:space-x-20 ">
                  <a href="#" @click="showPopUp('update_classroom','','','',classroom)" class="flex md:w-[100px] h-8 text-sm rounded-lg md:text-white md:bg-[#000000]" ><p class="flex pl-3 pt-1"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        ...
        <tbody v-else>
                  <td colspan="9" class="h-20">
                      <h3 class="text-lg text-center">No classroom found</h3>
                  </td>
        </tbody>
        </table>
        </div>
...
<template v-slot:entries>
  Show {{ firstItem }} to {{ lastItem }} of {{ getClassroomList.no_of_classroom }} courses
</template>
<template v-slot:pagination>
  <div v-if="getClassroomList.no_of_classroom" class="flex pt-6 text-end">
          <input v-set="noOfPages = Math.ceil(getClassroomList.no_of_classroom / formData.limit)" type="hidden">
  ...
  setup() {
     ...
     function selectPage(num,noOfPages){
        if(num <= noOfPages && num>0) 
        {
         ...
         function getClassroom(){
            ...
            return classroomStores.getClassroomList(school_id,page,limit)
        }

        const getClassroomList = computed(() => {
            return classroomStores.classroomList;
        });
    ...
    function print_pdf() {
        ...
        tag + `
          ...
          +`<br><br>
          <center>
            <h3 class='text-2xl'>Classroom List</h3>
            <hr>
            <br><br>
          </center>`;
    ...
    return {currentUser,getUsers,setNumberOfRecords,isLoading,selectPage,currentPage,sortRecords,isSorted,showPopUp,deleteAction,popupDetails,print_pdf,print_ms,formData,hideCols,popup_type,getClassroomList}
  }
}
</script>
