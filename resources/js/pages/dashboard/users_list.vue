...
<template >
      ...
      <div class="w-full md:w-5/6">
        ...
        <DataTable>
          ...
          <template v-slot:searchInput>
            <input type="text" class="w-full mt-1 h-[28px] md:h-10 ring-1 ring-[#000000] rounded-tl rounded-bl placeholder:p-1 placeholder:font-light  enabled:p-2" @keyup="clearSearch()" placeholder="Search user here" v-model="formData.user_name">
          <button @click="searchUser()" class="w-12 h-[30px] md:h-[42px] mt-[3px] text-sm text-white bg-[#000000] rounded-tr rounded-br"><p class="flex pl-2 pt-[1px]"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  <template v-slot:pagination>
    <div v-if="getUsersList.records" class="flex pt-6 text-end">
            <input v-set="noOfPages = Math.ceil(getUsersList.records / formData.limit)" type="hidden">
    ...
    setup() {
      ...
      watch(route,() => { })
       ...
       function clearSearch(){
        ...
        function searchUser(){
            return manageUsersStore.searchUsersAuthentications(formData.value)
        }
  
        function getUsersAuthentications(){
              return manageUsersStore.getUsersAuthentications(formData.value)
        }
          ...
          return uiStore.openPopUpFunc(popup_type,to,popup_records);    
           ...
           function toggleSideBar(){
            ...
            function removeCol(index){
                if(colsToShow.value.length > 4 && colsToShow.value[index].key !='name' )
                    colsToShow.value.splice(index, 1)
                else
